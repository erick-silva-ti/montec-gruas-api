...
function Header() {
  return (
    <View style={styles.headerWrap}>
      ...
      <Text style={styles.headerTitle}>Editar Orcamento</Text>
      <View style={{ width: 50 }} />
    </View>
  );
}
...
export default function EditQuoteScreen() {
  ...
  const { state, updateQuote } = useData();

  const quote = useMemo(() => state.quotes.find((entry) => entry.id === id), [state.quotes, id]);

  const [customerId, setCustomerId] = useState<string>(quote?.customerId ?? '');
  const [customerSearch, setCustomerSearch] = useState(quote?.customerName ?? '');

  const [notes, setNotes] = useState(quote?.notes ?? '');
  const [paymentCondition, setPaymentCondition] = useState(quote?.paymentCondition ?? '');
  const [executionDeadline, setExecutionDeadline] = useState(quote?.executionDeadline ?? '');
  ...
  const saveQuote = async () => {
    try {
      ...
      Alert.alert('Orcamento atualizado', `Numero ${updatedQuote.number} atualizado com sucesso.`);
      router.replace('/orcamentos');
    } catch (error) {
      Alert.alert('Nao foi possivel atualizar', (error as Error).message);
    }
  };
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={{ padding: 16 }}>
          <Text style={styles.empty}>Orcamento nao encontrado.</Text>
        </View>
      </SafeAreaView>
    );
  }
